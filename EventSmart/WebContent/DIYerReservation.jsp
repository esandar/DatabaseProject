...
<body style="background-image: url(https://images.unsplash.com/photo-1415025148099-17fe74102b28?ixlib=rb-0.3.5&ixid=eyJhcHBfaWQiOjEyMDd9&s=1d2697309ae275b27d03270feb9d73e6&auto=format&fit=crop&w=2736&q=80)">
	...
	<form action="diyerreservation" method="post">
		<h1 class="h3 mb-3 font-weight-normal" style="color: #e6f7ff">Search for Reservations by DIYer</h1>
		<p>
			<label for="diyername" class="sr-only">DIYerName</label>
			<input id="diyername" name="diyername" value="${fn:escapeXml(param.diyername)}" style="width: 400px" class="form-control" placeholder="UserName" required autofocus>
		</p>
		<p>
			<button
          	class="btn btn-outline-light my-2 my-sm-0" 
	...
	<br/>
	<br/>
	<h1 style="color: #e6f7ff">Matching Reservations</h1>
		<table class="table table-striped table-sm">
			<tr>
				<th style="color: #e6f7ff">PlannerName</th>
				<th style="color: #e6f7ff">EventDate</th>
				<th style="color: #e6f7ff">Update Reservation</th>
				<th style="color: #e6f7ff">Delete Reservation</th>
			</tr>
			<c:forEach items="${reservations}" var="reservation">
				<tr>
					<td style="color: #e6f7ff"><c:out value="${reservation.getPlannername()}" /></td>
					<td style="color: #e6f7ff"><c:out value="${reservation.getEventdate()}" /></td>
					<td style="color: #e6f7ff"><a href="updatereservation?reservationid=<c:out value="${reservation.getReservationID()}" />">Update</a></td>
					<td style="color: #e6f7ff"><a href="deletereservation?reservationid=<c:out value="${reservation.getReservationID()}" />">Delete</a></td>
				</tr>
			</c:forEach>
		</table>
	</div>
